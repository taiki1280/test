import random
from taiki import RUNNING_ID_DICT
import logging
...
def run(self):
    ...
    if RUNNING_ID_DICT[self.RUNNING_ID]["計算モード"]["ステップ"] == 1:
        ...
        if self.text.isdigit() == True:
            ...
            # RUNNING_ID_DICT[self.RUNNING_ID]["計算モード"]["ミス回数"] += 1
            self.mistake(RUNNING_ID_DICT[self.RUNNING_ID]["計算モード"]["ミス回数"], "数字", str(random.randrange(1, 101, 1)),
        ...
        if self.text in ["たす", "ひく", "かける", "わる", "あまり"]:
            ...
            # RUNNING_ID_DICT[self.RUNNING_ID]["計算モード"]["ミス回数"] += 1
            self.mistake(RUNNING_ID_DICT[self.RUNNING_ID]
