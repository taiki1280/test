from reply import Reply, RUNNING_ID_DICT
...
def run(self):
    ...
    if self.text in self.MODE_ARRAY:
        ...
        # 沈黙モードの場合
            ...
            elif self.MODE == "財布モード":
                if "財布モード" not in RUNNING_ID_DICT[self.RUNNING_ID]:
                    self.reply(
                        text, "財布の中身を記録しておくために数値を保持するだけのモードです。", "なお現在開発中の為、値の保持がしばしば出来なくなると思います。\nご了承ください。\nそうなった際にはお手数ですがトーク履歴より値を再入力してください。", "あなたの所持金を入力してください。", "例）0")
                else:
                    self.reply("前回までの所持金は「" + str(RUNNING_ID_DICT[self.RUNNING_ID]["財布モード"]["現在の所持金"] +
                                                  "円」です。", "値に変更がある場合は「数値」を入力後、「たす」か「ひく」か入力してください。", "なければ「財布モード終了」と入力してください"))
            else:
                self.reply(text)
            # ない場合はそのまま
